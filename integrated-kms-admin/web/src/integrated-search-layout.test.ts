...
test('integrated search links citation badges to score-sorted reference cards and source actions', () => {
  const source = readFileSync(resolve(import.meta.dir, 'features/IntegratedSearch.tsx'), 'utf8')
  ...
  expect(source).toContain('className="reference-toggle"')
  expect(source).toContain('className="reference-detail"')
  expect(source).toContain('aria-expanded={isExpanded}')
  expect(source).toContain('onReferenceSelect={expandReference}')
