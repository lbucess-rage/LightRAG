...
const terms = [
  {
    term: '고정 답변',
    description: '운영자가 검토하고 승인한 원문 또는 요약 답변입니다. 검색 시 LLM이 본문을 새로 쓰지 않고 이 답변 ID를 찾는 것이 목표입니다.',
  },
  {
    term: '소스',
    description: '답변 초안을 만들기 위해 입력하는 텍스트, HTML, Markdown, URL, 파일, DB/NoSQL 스냅샷 같은 원자료입니다.',
  },
  {
    term: '가이던스',
    description: '답변을 찾기 쉽게 만드는 질문, 키워드, 동의어, 메모입니다. 질문 유형은 높은 가중치로 매칭됩니다.',
  },
  {
    term: '제외어',
  ...
  {
    term: '리비전',
    description: '답변 저장 시점의 스냅샷입니다. 잘못 수정했을 때 이전 버전으로 복구할 수 있습니다.',
  },
  {
    term: '매칭 점수',
    description: '제목, 요약, 본문, 태그, 가이던스, 우선순위가 질문과 얼마나 맞는지 합산한 결정적 점수입니다.',
  },
  {
    term: '구조화 프로파일',
    description: 'JSON, CSV, DB/NoSQL 스냅샷처럼 필드와 행으로 해석 가능한 소스의 컬럼, 샘플 행, 행 수 정보를 보존한 값입니다.',
  },
  {
    term: 'Safe Query',
    description: '운영자가 구조화 스냅샷을 조건 검색하기 위한 제한된 쿼리입니다. 현재는 등록된 답변 스냅샷 범위에서 미리보기와 실행을 지원합니다.',
  },
  {
    term: '분석 이벤트',
    description: '질문이 어떤 답변으로 매칭됐는지, 후보 점수와 처리 지연이 어땠는지를 남긴 조회 이력입니다.',
  },
  {
    term: '신뢰도',
...
const screenshotGuides = [
  {
    title: '답변 라이브러리',
    caption: '답변을 만들고, 게시하고, 상세 편집과 리비전 복구를 수행하는 운영 화면입니다.',
    tab: 'answers' as AppTab,
  },
  {
    title: '소스 위저드',
    caption: '원자료를 미리보기로 분석하고 제목, 요약, 태그, 가이던스 후보를 검토한 뒤 답변 초안으로 바꾸는 화면입니다.',
  ...
  {
    title: '구조화 데이터',
    caption: '소스 프로파일의 필드와 샘플 행을 확인하고 Safe Query를 미리보기/실행하는 화면입니다.',
  ...
  {
    title: '분석',
    caption: '조회 이벤트, 미매칭, 평균 지연, 상위 선택 답변을 확인하고 운영 보강 대상을 찾는 화면입니다.',
...
export default function AnswerHelp() {
  ...
  const tutorialSteps = useMemo<GuideStep[]>(() => [
    ...
    },
    {
      ...
      description: t('answerCatalog.help.steps.review.description', '본문, 승인 요약, 표시 방식, 우선순위, 유효기간, 태그를 확인합니다. 잘못 저장해도 리비전에서 복구할 수 있습니다.'),
      tab: 'answers',
    ...
    },
    {
      ...
      description: t('answerCatalog.help.steps.structured.description', 'JSON, CSV, DB/NoSQL 스냅샷에서 감지된 필드와 샘플 행을 확인하고, Safe Query로 조건 검색을 미리보기/실행합니다.'),
      tab: 'structured-data',
    ...
    },
    {
      ...
      description: t('answerCatalog.help.steps.publish.description', '충분히 검증된 답변만 게시합니다. 운영 중에는 조회 이벤트, 미매칭 질문, 평균 지연, 상위 선택 답변을 보고 가이던스를 보강합니다.'),
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5 p-4">
        <section className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
              {t('answerCatalog.help.description', '운영자가 고정 답변을 만들고, 매칭 기준을 관리하고, 실제 질문에서 어떤 답변이 선택되는지 검증하는 흐름을 단계별로 안내합니다.')}
            </p>
          </div>
        ...
        <section className="grid gap-4 xl:grid-cols-[420px_1fr]">
          <div className="rounded-md border p-4">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="font-semibold">{t('answerCatalog.help.tutorial', '단계별 튜터')}</h2>
            </div>
        ...
        <section>
          <div className="mb-3 flex items-center gap-2">
            ...
            <Badge variant="outline">{t('answerCatalog.help.replaceable', '나중에 실제 스크린샷으로 교체 가능')}</Badge>
          </div>
        ...
        <section className="rounded-md border p-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <ScoreHint icon={<SlidersHorizontalIcon className="h-4 w-4" />} title="애매한 점수" text="후보가 여러 개 비슷하게 뜨면 가이던스를 더 구체화하거나 제외어를 추가합니다." />
            <ScoreHint icon={<FileInputIcon className="h-4 w-4" />} title="미매칭" text="정답 후보가 없으면 소스 초안을 새로 만들거나 기존 답변의 질문/키워드를 보강합니다." />
            <ScoreHint icon={<SearchCheckIcon className="h-4 w-4" />} title="Safe Query" text="구조화 데이터는 먼저 SQL 모양을 미리보고, 조건과 반환 행 수가 맞을 때만 실행합니다." />
          </div>
        </section>
      </div>
    </div>
